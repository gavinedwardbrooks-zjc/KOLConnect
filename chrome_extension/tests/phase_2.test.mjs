...
// M8.3-T retires active detail-page requests. A blocked passive session must
// fail closed before any discovery or fetch; full layer tests live in test_m8_3_t.
const savedTikTokChrome = globalThis.chrome;
globalThis.chrome = { scripting: { executeScript: async ({ world }) => {
  assert.equal(world, "ISOLATED");
  return [{ result: { stopped: "CAPTCHA_DETECTED" } }];
} } };
await assert.rejects(TikTok.collectRecentContent(1, {
  analysisUrl: "https://www.tiktok.com/@fixture_creator",
}), /CAPTCHA_DETECTED/);
globalThis.chrome = savedTikTokChrome;
const tiktokSource = readFileSync(new URL("../platform/tiktok.js", import.meta.url), "utf8");
assert.doesNotMatch(tiktokSource, /fetchTikTokContentDetail|fetch\(/);
...
for (const forbidden of ["interceptor", "adapter", "collector"]) {
  assert.equal(manifestText.includes(forbidden), false);
}
const passiveEntry = manifest.content_scripts.find(entry => entry.js.includes("content/passive_capture_bridge.js"));
assert.ok(passiveEntry.matches.every(url => url.includes("tiktok.com")));
