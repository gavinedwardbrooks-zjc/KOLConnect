...
def _match_sender(connection, address: str) -> tuple[str, str | None, str | None]:
    ...
    if any(not row["owner_id"] for row in rows):
        return "unassigned_account", None, None
    owners = {row["creator_id"] for row in rows}
    if len(owners) != 1:
        return "ambiguous", None, None
    return ("matched" if len(rows) == 1 else "matched_multi_account", next(iter(owners)),
            rows[0]["account_uid"] if len(rows) == 1 else None)


def _persist_observation(factory, account_id: str, mailbox_id: str, uidvalidity: str,
                         uid: str, raw: bytes, observed_at: str, own_addresses: set[str]) -> tuple[bool, dict]:
    ...
    direction = "inbound" if sender and own_addresses and sender not in own_addresses else "unknown"
    with factory.write_transaction() as connection:
        ...
        match, creator_id, account_uid = _match_sender(connection, sender) if direction == "inbound" else ("unmatched", None, None)
        ...
        connection.execute(
            ...
            (fact_id, account_id, direction, message.get("Message-ID"), message.get("In-Reply-To"),
             ...
             date, observed_at, sender or None, match, creator_id, account_uid),
        )
        connection.execute(
...
def sync_inbox(account: dict, factory, *, imap_factory=None, now: datetime | None = None) -> dict:
    """Persist only observed Inbox facts; caller may project returned rows into the old UI cache."""
    ...
    mailbox_id = f"mailbox_{hashlib.sha256((account_id + ':INBOX').encode()).hexdigest()[:32]}"
    with factory.write_transaction() as connection:
        ...
        connection.execute("INSERT OR IGNORE INTO mailbox_sync_states(mailbox_id,mail_account_id,folder_name,role) VALUES (?,?,?,?)",
                           (mailbox_id, account_id, "INBOX", "inbox"))
    ...
    try:
        ...
        for uid in selected:
            ...
            created, fact = _persist_observation(factory, account_id, mailbox_id, validity, uid, raw, observed_at, own_addresses)
            if created:
        ...
        return {"mail_account_id": account_id, "mailbox_id": mailbox_id, "uidvalidity": validity,
                ...
                "history_coverage": coverage, "high_water_uid": high_water}
    finally:
        try:
            mailbox.logout()
        except Exception:
